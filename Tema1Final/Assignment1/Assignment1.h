...
float getDiscountedPrice(AccountType accountType, float initialPrice, float discountAsPercentage)
{
	...
	if (discountAsPercentage == 0) { return initialPrice; }
...
void filterValuesBiggerThanVersion2(const int* valori, int nrValori, int valoarePrag, int** valoriMaiMari)
{
	...
	if (sizeControler <= 0) { *valoriMaiMari = nullptr; }
	else {
		*valoriMaiMari = new int[sizeControler];
		int elementControler = 0;
		int* helper = *valoriMaiMari;
		for (int i = 0; i < nrValori; i++) {
			if (valori[i] >= valoarePrag) {
				helper[elementControler] = valori[i];
				elementControler += 1;
			}
		}
		*valoriMaiMari = helper;
	}
}
